...
- If the user asks for 自媒体三件套 and has already chosen `先生成文案和标题`, output titles plus body first, then ask the user to choose or revise one final title before cover generation.
- If the user asks for 自媒体三件套 and has chosen `直接生图并同时给标题文案`, output titles plus body alongside the 3 vertical cover candidates. Later horizontal adaptation should use the selected cover as the visual source, not wait for a final title decision.
...
- **media-kit-copy-first**: `自媒体三件套` + 用户选择 `先生成文案和标题`; generate titles and body, then request one confirmed title before image generation.
- **media-kit-direct**: `自媒体三件套` + 用户选择 `直接生图`; generate titles, body, and 3 vertical covers in the same delivery.
